...
export async function ensureFavoritesConversation(address: string) {
  ...
  let conversation = await prisma.conversation.findFirst({
    where: { kind: ConversationKind.favorites, ownerId: user.id },
  });
...
export async function ensureOrderConversation(
  ...
  if (!conversation) {
    ...
    await prisma.conversationParticipant.upsert({
      ...
      update: { role: ParticipantRole.maker, userId: makerUser.id },
      create: {
        conversationId: conversation.id,
        address: normalizedMaker,
        role: ParticipantRole.maker,
    ...
    if (normalizedTaker && takerUser) {
      await prisma.conversationParticipant.upsert({
        ...
        update: { role: ParticipantRole.taker, userId: takerUser.id },
        create: {
          conversationId: conversation.id,
          address: normalizedTaker,
          role: ParticipantRole.taker,
